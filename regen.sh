...
cd cmd/signcert && CGO_ENABLED=0 go build && ./signcert -key1 ../../tls/ca/key.pem -key2 ../../tls/ca/key2.pem -cert ../cagen/Verisign.pem > ../../tls/ca/cross-cert1.pem && cd -

openssl x509 -req -days 500 -set_serial 01 \
   -in tls/server/server-req.pem \
   -out tls/server/cert.pem \
   -CA tls/ca/cross-cert1.pem \
   -CAkey tls/ca/key.pem

cat tls/server/cert.pem tls/ca/cross-cert1.pem > tls/server/cert-bundle.pem

# Verify
echo "--- Verify with cert 1"
openssl verify -verbose -CAfile tls/ca/cacert.pem tls/server/cert-bundle.pem
echo "--- Verify with cert 2"
openssl verify -verbose -CAfile tls/ca/cacert2.pem tls/server/cert-bundle.pem
